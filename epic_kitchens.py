...
# #####################################################
# class EpicVideoClsDataset_ssvli(Dataset):
#     """Load your own video classification dataset."""
...
#             raise ImportError("Unable to import `decord` which is required to read videos.")
#         if mode == 'train':
#             print(f"Loading {mode} texts pt file...")
#             self.texts = torch.load('../../home/welles/scratch/datasets/SSVLI/EPIC_100_train_noun_text.pt')
#         elif mode == 'validation':
#             print(f"Loading {mode} texts pt file...")
#             self.texts = torch.load('../../home/welles/scratch/datasets/SSVLI/EPIC_100_val_noun_text.pt')
#         elif mode == 'test':
#             print(f"Loading {mode} texts pt file...")
#             self.texts = torch.load('../../home/welles/scratch/datasets/SSVLI/EPIC_100_test_noun_text.pt')
...
#                 return frame_list, text_list, label_list, index_list, {}
#             else:
#                 buffer = self._aug_frame(buffer, args)
            
#             return buffer, video_text, self.label_array[index], index, {}
...
#                     buffer = self.loadvideo_decord(sample)
#             buffer = self.data_transform(buffer)
#             return buffer, video_text, self.label_array[index], sample.split("/")[-1].split(".")[0]
...
#             buffer = self.data_transform(buffer)
#             return buffer, video_text, self.test_label_array[index], sample.split("/")[-1].split(".")[0], \
#                    chunk_nb, split_nb
#         else:
#             raise NameError('mode {} unkown'.format(self.mode))
