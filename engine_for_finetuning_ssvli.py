...
def final_test(data_loader, model, device, file, args=None):
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        for i in range(output.size(0)):
            string = "{} {} {} \n".format(ids[i], \
                                                str(output.data[i].cpu().argmax().numpy().tolist()), \
...
def merge(eval_path, num_tasks, args=None):
    dict_feats = {}
    dict_label = {}
    dict_pos = {}
...
##################################################
def train_class_batch_BB_focused(model, samples, bboxs, target, criterion):
    outputs = model(samples, bboxs)
    loss = criterion(outputs, target)
    return loss, outputs
...
def train_one_epoch_BB_focused(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for data_iter_step, (samples, bboxs, targets, _, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        step = data_iter_step // update_freq
        if step >= num_training_steps_per_epoch:
            continue
        it = start_steps + step  # global training iteration
        ...
        if loss_scaler is None:
            samples = samples.half()
            loss, output = train_class_batch_BB_focused(
                model, samples, bboxs, targets, criterion)
        else:
            with torch.cuda.amp.autocast():
                loss, output = train_class_batch_BB_focused(
                    model, samples, bboxs, targets, criterion)
...
@torch.no_grad()
def final_test_BB_focused(data_loader, model, device, file, args=None):
    criterion = torch.nn.CrossEntropyLoss()
